...
const RentModal = () => {
        ...
        const onNext = () => {
                if ((step === STEPS.CATEGORY && !category) || (step === STEPS.LOCATION && !location)) {
                        toast.error('Please select and fill in all fields');
                        return;
                }
                setStep((value) => value + 1);
        };
        ...
        if (step === STEPS.LOCATION) {
                bodyContent = (
                        <div className="flex flex-col gap-8">
                                <Heading title="Where is your place located?" subtitle="Help the guests find you!" />
        ...
        if (step === STEPS.INFO) {
                bodyContent = (
                        <div className="flex flex-col gap-8">
                                ...
                                <Counter title="Number of guests" subtitle="How many guests do you allow?" value={guestCount} onChange={(value) => setCustomValue('guestCount', value)} />
                                <hr />
        ...
        if (step === STEPS.IMAGES) {
                bodyContent = (
                        <div className="flex flex-col gap-8">
                                <Heading title="Add some photos of your place" subtitle="Help the guests get a feel for your place!" />
                                <ImageUpload
                                        value={imageSrc}
                                        onChange={(value) => {
        ...
        if (step === STEPS.PRICE) {
                bodyContent = (
                        <div className="flex flex-col gap-8">
                                ...
                                <Input id="price" label="Price" formatPrice type="number" disabled={isLoading} errors={errors} required register={register} />
                        </div>
                );
        }
